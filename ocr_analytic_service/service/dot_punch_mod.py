...
def dot_punched_data_parser(imgobj):
    ...
    logger.info('Dot model path in plp: %s' % os.path.join(base_path, '/models/model_final_dotpunch.pth'))
    ...
    try:
        str_ocr, conf, conf_band = Inference().output(
            data, prediction, imgobj)
        out_obj = {}
